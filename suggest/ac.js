...
// this function controls background color of marked row and results color( mozilla + IE )

// c.displaySpan.style.color=resultColor defines color for ' x results'
// c.style.backgroundColor=markedRowColor defines background color for marked row
// c.style.color=suggestColor defines color of suggest
// c.style.fontFamily=textFont defines font of suggestion and result
function l(c,name){db();c.className=name;if(Ka){return}switch(name.charAt(0)){case "m":c.style.fontSize="13px";c.style.textAlign="left";c.style.fontFamily=textFont;c.style.wordWrap="break-word";break;case "l":c.style.display="block";c.style.paddingLeft="3";c.style.paddingRight="3";c.style.height="16px";c.style.overflow="hidden";break;case "a":c.style.backgroundColor="white";c.style.color=suggestColor;if(c.displaySpan){c.displaySpan.style.color=resultsColor}break;case "b":c.style.backgroundColor=markedRowColor;c.style.color="white";if(c.displaySpan){c.displaySpan.style.color="white"}break;case "c":c.style.width=p+"%";c.style.cssFloat="left";break;case "d":c.style.cssFloat="right";c.style.width=100-p+"%";if(w=="query"){c.style.fontSize="10px";c.style.textAlign="right";c.style.color="#DF1111";c.style.paddingTop="3px"}else{c.style.color="#696969"}break}}
function db(){p=65;if(w=="query"){var wb=110;var Sa=Ja();var tb=(Sa-wb)/Sa*100;p=tb}else{p=65}if(ya){p=99.99}}
function Gb(i){db();var Ub="font-size: 13px; font-family: arial,sans-serif; word-wrap:break-word;";var Vb="display: block; padding-left: 3; padding-right: 3; height: 16px; overflow: hidden;";var bc="background-color: white;";var qb="background-color: #3366cc; color: green ! important;";var ub="display: block; margin-left: 0%; width: "+p+"%; float: left;";var Ga="display: block; margin-left: "+p+"%;";if(w=="query"){Ga+="font-size: 10px; text-align: right; color: green; padding-top: 3px;"}else{Ga+="color: #696969;"}D(".mAutoComplete",Ub);D(".lAutoComplete",Vb);D(".aAutoComplete *",bc);D(".bAutoComplete *",qb);D(".cAutoComplete",ub);D(".dAutoComplete",Ga);l(i,"mAutoComplete")}
function rb(i,cs,Hb){while(i.childNodes.length>0)i.removeChild(i.childNodes[0]);for(var f=0;f<cs.length;++f){var u=document.createElement("DIV");l(u,"aAutoComplete");u.onmousedown=Cb;u.onmouseover=pb;u.onmouseout=ec;var ka=document.createElement("SPAN");l(ka,"lAutoComplete");var ua=document.createElement("SPAN");ua.innerHTML=cs[f];var ea=document.createElement("SPAN");l(ea,"dAutoComplete");l(ua,"cAutoComplete");u.displaySpan=ea;if(!ya&&navigator&&navigator.userAgent.toLowerCase().indexOf("msie")==-1)ea.innerHTML=Hb[f] + " results";ka.appendChild(ua);ka.appendChild(ea);u.appendChild(ka);i.appendChild(u)}}
...
// change here to get rid of the wp-setting.php output
